...
class MeasureBoardRepository
    ...
    constructor(
        ...
        private fun checkStatues(result: String) {
            when (getResultType(result)) {
                ...
                ONGOING_HEARTBEAT -> {
                    ...
                    _tripStatus.value =
                        MCUTripStatus.Ongoing(
                            isPaused = isStopped,
                            isLocked = overSpeedLockupDuration > 0,
                            fare = BigDecimal(fare.format("%.1F")).divide(BigDecimal("100")).toDouble(),
                            totalFare = BigDecimal(totalFare.format("%.1F")).divide(BigDecimal("100")).toDouble(),
                            extra = BigDecimal(extras.format("%.1F")).divide(BigDecimal("100")).toDouble(),
                            distanceInMeter = BigDecimal(distance.format("%.0F")).multiply(BigDecimal("10")).toDouble(),
        ...
        private fun readMeasureBoardDeviceId(): Boolean = mBusModel?.write(MeasureBoardUtils.getMeasureBoardDeviceIdCmd()) ?: false

        fun writeMeasureBoardData(licensePlate: String): Boolean =
            mBusModel?.write(MeasureBoardUtils.getWritingDataIntoMeasureBoardCmd(licensePlate)) ?: false

        fun unlockMeter(): Boolean = mBusModel?.write(MeasureBoardUtils.getUnlockCmd()) ?: false
        ...
        private fun getTimeInSeconds(duration: String): Long =
        ...
        private fun String.format(pattern: String): String = String.format(pattern, this)
